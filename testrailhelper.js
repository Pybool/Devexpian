...
function findObjectByNameAndParentId(objects, name, parentId) {
  for (let i = 0; i < objects.length; i++) {
      if (objects[i].parent_id == parentId && objects[i].name.trim() == name.trim()) {
        return objects[i]
      }
  }
}
  ...
  if (test.length > 0) {
    ...
    for (let i = 0; i < genratedCase.length; i++) {
      ...
      if (cases.length > 0) {
        ...
        await publishTestRun(cases[0].id, genratedCase[i][1], runId,opt,options)
      } else {
        try {
          ...
          await publishTestRun(resp.data.id, genratedCase[i][1], runId,opt,options)
        } catch (err) {
          console.log(err)
        }
      }
...
async function publishTestRun(caseIds, statusId, runId,opt,options) {
  var statId
  if (statusId) {
    statId = 1
  } else {
